...
function buildMode:Init(dbFileName, buildName, buildXML, convertBuild, importLink)
	...
	self.controls.modeCustom = new("ButtonControl", {"LEFT",self.controls.modeParty,"RIGHT"}, {4, 0, 72, 20}, "Gems Info", function()
		self.viewMode = "CUSTOM"
	end)
	self.controls.modeCustom.locked = function() return self.viewMode == "CUSTOM" end
	self.controls.modeCustomNew = new("ButtonControl", {"LEFT",self.controls.modeCustom,"RIGHT"}, {4, 0, 72, 20}, "Custom", function()
		self.viewMode = "CUSTOM_NEW"
	end)
	self.controls.modeCustomNew.locked = function() return self.viewMode == "CUSTOM_NEW" end
	-- Skills
	...
	self.gemsInfoTab = new("GemsInfoTab", self)
	...
	self.savers = {
		...
		["Notes"] = self.notesTab,
		["Party"] = self.partyTab,
		["Custom"] = self.gemsInfoTab,
		["CustomNew"] = self.customTab,
		["Tree"] = self.treeTab,
...
function buildMode:ResetModFlags()
	...
	self.gemsInfoTab.modFlag = false
	self.customTab.modFlag = false
end
...
function buildMode:OnFrame(inputEvents)
	...
	elseif self.viewMode == "CUSTOM" then
		self.gemsInfoTab:Draw(tabViewPort, inputEvents)
	elseif self.viewMode == "CUSTOM_NEW" then
		self.customTab:Draw(tabViewPort, inputEvents)
	end

	self.unsaved = self.modFlag or self.notesTab.modFlag or self.partyTab.modFlag or self.configTab.modFlag or self.treeTab.modFlag or self.treeTab.searchFlag or self.spec.modFlag or self.skillsTab.modFlag or self.itemsTab.modFlag or self.calcsTab.modFlag or self.gemsInfoTab.modFlag or self.customTab.modFlag
